...
class FRVSRGan(SuperResolution):
  ...
  def train(self, inputs, labels, learning_rate=None):
    ...
    for lr, hr in zip(frames, labels):
      sr, hrw, lrw, flow = self.frvsrgan(lr, last_lr, last_sr.detach())
      last_lr = lr
      last_sr = sr
      l2_image = F.mse_loss(sr, hr)
      l2_warp = F.mse_loss(lrw, lr)
      tv_flow = total_variance(flow)
      one_gan_loss = self.gan.get_adv_loss(sr, hr)
  ...
  def eval(self, inputs, labels=None, **kwargs):
    ...
    for lr in frames:
      ...
      sr, _, _, _ = self.frvsrgan(lr, last_lr, last_sr)
      last_lr = lr.detach()
      last_sr = sr.detach()
